...
interface ClientV2MapperExtensions {
    ...
    static com.concordium.sdk.responses.nodeinfo.NodeInfo to(NodeInfo nodeInfo) {
        ...
        // Node is BOOTSTRAPPER
        if (!nodeInfo.hasNode()) {return getNodeInfoForBootstrapper(builder);}

        builder = builder.peerType(PeerType.NODE);

        // Node is not configured with baker keys
        if (!nodeInfo.getNode().hasActive()) {return getNodeInfoForPassive(builder);}
    ...
    static com.concordium.sdk.responses.nodeinfo.NodeInfo getNodeInfoForBootstrapper(com.concordium.sdk.responses.nodeinfo.NodeInfo.NodeInfoBuilder builder) {
        builder = builder.peerType(PeerType.BOOTSTRAPPER)
                .consensusState(ConsensusState.NOT_RUNNING)
                .bakingStatus(BakingStatus.NOT_IN_COMMITTEE)
                .committeeDetails(null);
        return builder.build();
    }

    // Helper method for getNodeInfo. Builds NodeInfo for Node configured without baker keys
    static com.concordium.sdk.responses.nodeinfo.NodeInfo getNodeInfoForPassive(com.concordium.sdk.responses.nodeinfo.NodeInfo.NodeInfoBuilder builder) {
        builder = builder.consensusState(ConsensusState.PASSIVE)
                .bakingStatus(BakingStatus.NOT_IN_COMMITTEE)
                .committeeDetails(null);
        return builder.build();
    }

    // Helper method for getNodeInfo. Builds NodeInfo for Node configured with baker keys not active in committee
    static com.concordium.sdk.responses.nodeinfo.NodeInfo getNodeInfoForNotActiveBaker(NodeInfo nodeInfo, com.concordium.sdk.responses.nodeinfo.NodeInfo.NodeInfoBuilder builder) {
        var passiveCommitteeInfo = nodeInfo.getNode().getActive().getPassiveCommitteeInfo();

        // Constants for comparing passiveCommitteeInfo
        val notInCommittee = NodeInfo.BakerConsensusInfo.PassiveCommitteeInfo.NOT_IN_COMMITTEE;
        val addedButNotActiveInCommittee = NodeInfo.BakerConsensusInfo.PassiveCommitteeInfo.ADDED_BUT_NOT_ACTIVE_IN_COMMITTEE;
        val addedButWrongKeys = NodeInfo.BakerConsensusInfo.PassiveCommitteeInfo.ADDED_BUT_WRONG_KEYS;
        ...
        if (passiveCommitteeInfo.equals(addedButWrongKeys)) {
            builder = builder.bakingStatus(BakingStatus.ADDED_BUT_WRONG_KEYS);
        }
        builder = builder.committeeDetails(null);
        return builder.build();
    }

    // Helper method for getNodeInfo. Builds NodeInfo for Node ACTIVE_IN_COMMITTEE
    static com.concordium.sdk.responses.nodeinfo.NodeInfo getNodeInfoForActiveBaker(NodeInfo nodeInfo, com.concordium.sdk.responses.nodeinfo.NodeInfo.NodeInfoBuilder builder) {
        builder = builder.bakingStatus(BakingStatus.ACTIVE_IN_COMMITTEE)
                .committeeDetails(committeeDetails(nodeInfo.getNode().getActive()));

        return builder.build();
    }

    // Converts BakerConsensusInfo from ACTIVE_IN_COMMITTEE Node to BakingCommitteeDetails
    // Prerequisite: bakerInfo must be from node that is ACTIVE_IN_COMMITTEE
    static BakingCommitteeDetails committeeDetails(NodeInfo.BakerConsensusInfo bakerInfo) {

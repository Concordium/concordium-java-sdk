...
import com.concordium.sdk.exceptions.AddressFormatException;
...
public class Base58 {
    ...
    public static String encodeChecked(int version, byte[] payload) {
        ...
        byte[] checksum = Arrays.copyOfRange(SHA256.hashTwice(addressBytes, 0, addressBytes.length-4), 0, 4);
     ...
     */
    public static byte[] decodeChecked(int version, String input) throws AddressFormatException {
        byte[] decoded  = decode(input);
        byte versionByte = decoded[0];
        if (versionByte != version) {
            throw new AddressFormatException.InvalidVersion();
        }
        if (decoded.length < 4) {
        ...
        byte[] checksum = Arrays.copyOfRange(SHA256.hashTwice(data), 0, 4);
